...
class ColdSurgeryUnavailableError(PreflightError):
    """The rootfs UID-1000 cold-surgery cannot run — detected BEFORE any
    destructive write, so the SD card is left UNTOUCHED (no partition wipe,
    no "Format?" popup). Raised when the bundled e2fsprogs tools are missing,
    won't load, or cannot open the target drive (e.g. errno 21
    ERROR_NOT_READY from a Cygwin device-path problem)."""

    def __init__(self, detail: str) -> None:
        super().__init__(detail)
        self.recovery_hint = (
            "The bundled ext4 tools (debugfs.exe / e2fsck.exe) could not be "
            "used to personalize the card's account, so the flash was aborted "
            "BEFORE touching the card — it is unchanged. Rebuild the app with "
            "vendor/ populated (see vendor/README.md), or clear the "
            "username/password fields in Customize to flash without the "
            "account rename."
        )
