...
# 1. Read /etc/passwd, modify pi → testuser + /home/pi → /home/testuser
# 2. Write back modified passwd (rm old, write new)
# 3. Same for shadow + group (just passwd is enough to prove pattern)
# 4. Rename /home/pi → /home/testuser (try debugfs 'rename' command)
...
echo "=== STEP 2: read /etc/passwd, modify with sed (pi → testuser), write back ==="
debugfs -R 'cat /etc/passwd' "$F" 2>/dev/null | grep -v "^debugfs" > /tmp/passwd_orig
echo "ORIGINAL:"
cat /tmp/passwd_orig
echo ""
sed 's|^pi:x:1000:1000:,,,:/home/pi:|testuser:x:1000:1000:,,,:/home/testuser:|' /tmp/passwd_orig > /tmp/passwd_new
echo "NEW:"
cat /tmp/passwd_new
echo ""
...
echo "=== STEP 4: try rename /home/pi → /home/testuser (does debugfs have mv?) ==="
cat > /tmp/rename.txt <<EOF
rename /home/pi /home/testuser
quit
EOF
