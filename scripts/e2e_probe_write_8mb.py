"""Quick probe — does write-at-offset-8MB succeed after DeleteVolumeMountPoint dance?

Reproduces the smallest failing step of the full E2E run: dismount the SD,
open the physical drive, write 1 MB at offset 8 MB (the old FAT32 start —
where the full flash hit ERROR_ACCESS_DENIED). If this probe succeeds,
the full flash should too.

NON-DESTRUCTIVE — only touches a single 1 MB region of the SD that already
...
def main() -> int:
    ...
    try:
        OFFSET = 8 * 1024 * 1024  # 8 MB — where the failing run died
        CHUNK = 1 << 20            # 1 MB
        print(f"\n== Probe: read 1 MB at offset {OFFSET} ==")
        original = dev.read(OFFSET, CHUNK)
        print(f"  read OK — {len(original)} bytes, first 4 bytes: {original[:4].hex()}")

        # Write the SAME bytes back — non-destructive but exercises the write path
        ...
        match = echo == original
        print(f"  re-read OK — bytes match: {match}")
        ...
        if target.drive_letters:
            ...
            ok = pio.attach_letter_to_unmounted_volume(target.drive_letters[0])
