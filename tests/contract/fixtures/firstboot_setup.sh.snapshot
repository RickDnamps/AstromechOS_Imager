    ...
    if [ -n "$BOOT_SSID" ]; then
        ...
        if [ "$SLAVE_OK" -ne 1 ]; then
            ...
            if [ -z "$FINAL_SSID" ] || [ "$FINAL_SSID" = "$BOOT_SSID" ]; then
                ...
                # CMD-1 fix 2026-05-30: FINAL_PSK comes from the Imager-baked
                # /boot/astromech_init.cfg [hotspot] password (operator-chosen
                # at flash time). FINAL_SSID is generated by gen_hotspot_ssid.sh
