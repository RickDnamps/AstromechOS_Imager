"""WP6 part 2 - selection safety: source-disk SAFETY STOP + provenance stamp.

Pins audit defect C1's hard backstop (the target disk must never be the disk
hosting the source image - the operator's USB SSD passes the eligibility
filter) and the provenance fields the GUI never used to set (the generated
/boot header read 'Generated by AstromechOS Imager  on ' with blanks).
"""
...
def test_cli_defaults_lockstep_with_gui():
    """Audit C4: --install-user default was 'pi', contradicting the GUI's
    locked 'astromech' invariant - a CLI flash produced a different account
